...
**Plans**: 4 plans
- [x] 12-01-PLAN.md — Built packages/achilles-skill (private workspace package) + embedded companion.md prompt (PROMPT-01..05); companionPromptPath + SKILL_PROMPTS_DIR exports as single source of truth; phase-12-unit Vitest project wired
- [x] 12-02-PLAN.md — Built apps/achilles/src/main/sandwich-defence.ts (SAFE-04 transcript delimiter wrap + manipulation-token detection) + normalisation.ts (PITFALLS #16 + #21 — strip ANSI, mask absolute paths, redact secret prefixes, drop fenced code, 600-char cap) + normalisation-fixtures.ts adversarial generators
- [x] 12-03-PLAN.md — Built renderer audio infrastructure (downsample-worklet 48k -> 16k Int16, mic-capture with pause/resume gating, playback-queue with PROMPT-04 single-entry-point enforcement, analyser-binding replacing Phase 11 MockAnalyser) + 6 new IPC channels + .strict() Zod schemas
- [x] 12-04-PLAN.md — Built apps/achilles/src/main/session.ts orchestrator composing voice-stt + claude-code-bridge + voice-tts + sandwich-defence + normalisation; LOOP-05 half-duplex gating + 300 ms debounce; PROMPT-05 runtime override via deriveOutcome; key-source.ts (store + env fallback); mock-loop-clients deterministic fakes; MOCK_LOOP=1 gated integration test; main/index.ts replaces Plan 11-01 mock timers (Plan 12-04 — 30 min, 3 tasks, 17 files, 113 new tests; phase-12-unit 200/200 under MOCK_LOOP=1)
**UI hint**: yes
