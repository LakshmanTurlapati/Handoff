...
**Plans**: 4 plans
- [ ] 12-01-PLAN.md — Build packages/achilles-skill (private workspace package) + the embedded companion system prompt at skill/prompts/companion.md driving PROMPT-01..03 + PROMPT-05; ship companionPromptPath + SKILL_PROMPTS_DIR exports as the single source of truth for both --append-system-prompt-file (Phase 12 orchestrator) and Phase 13 SKILL.md; wire phase-12-unit Vitest project
- [ ] 12-02-PLAN.md — Build apps/achilles/src/main/sandwich-defence.ts (SAFE-04 transcript delimiter wrap + manipulation-token detection) + normalisation.ts (PITFALLS #16 + #21 — strip ANSI, mask absolute paths, redact secret prefixes, drop fenced code, 600-char cap) + normalisation-fixtures.ts adversarial generators (deterministic composition over verbatim trigger strings)
- [ ] 12-03-PLAN.md — Build renderer audio infrastructure (downsample-worklet 48k -> 16k Int16, mic-capture with pause/resume gating, playback-queue with PROMPT-04 single-entry-point enforcement, analyser-binding replacing Phase 11 MockAnalyser) + 6 new IPC channels + .strict() Zod schemas (TTS chunk fan-out, playback complete, utterance commit, mic frame, STT token request/response)
- [ ] 12-04-PLAN.md — Build apps/achilles/src/main/session.ts orchestrator composing voice-stt + claude-code-bridge + voice-tts + sandwich-defence + normalisation; LOOP-05 half-duplex gating + 300 ms debounce; PROMPT-05 runtime override via deriveOutcome; key-source.ts (store + env fallback); mock-loop-clients deterministic fakes; MOCK_LOOP=1 gated integration test; main/index.ts replaces Plan 11-01 mock timers
**UI hint**: yes
