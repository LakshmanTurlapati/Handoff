...
- [x] **LOOP-05**: TTS audio chunks play back in arrival order via the renderer's AudioContext; the mic is gated during playback (half-duplex) and re-enabled ~300 ms after the last audio chunk drains. (Plan 12-04 — apps/achilles/src/main/session.ts orchestrator owns the half-duplex sequence: pauseFrameDelivery() on processing → speaking transition, MIC_FRAME drops during speaking via metrics.framesDroppedDuringSpeaking, locked SPEAKING_DEBOUNCE_MS = 300 timer between TTS_PLAYBACK_COMPLETE and state idle; the renderer-side playback-queue from Plan 12-03 owns the arrival-order rendering via the SequenceBuffer-shape ordering inside playback-queue.ts.)
...
- [x] **PROMPT-05**: When the Claude Code run errored (non-zero exit, tool errors, refused-permission), the spoken completion is overridden with an honest "I ran into a problem" message derived from the exit code + `tool_result` events, regardless of what the LLM narrated. (Plan 12-04 — session.ts inspects session.outcome derived via @achilles/claude-code-bridge's deriveOutcome at process_exit; on failure the spoken summary becomes "I ran into a problem. <reason>" with reason ∈ {exit_code: <N>, tool_error, cancelled}. Test SE6/SE7 + integration EE2 pin the override. The FAILURE_OVERRIDE_PREFIX constant in session.ts is mirrored by the prompt body in packages/achilles-skill/skill/prompts/companion.md; drift causes either test to fail.)
