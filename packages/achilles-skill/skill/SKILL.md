...
Half-duplex turn-taking gates the microphone while text-to-speech is playing so the agent does not transcribe its own voice back into the next user turn. The orchestrator in the local Achilles app owns the audio gate and the failure-detection path; the skill body merely teaches Claude how to launch the orchestrator and what the spoken contract looks like.
...
The launch is non-blocking. Claude Code's interactive session is left undisturbed. You may continue to handle other tool calls in the same session after the launch returns; the running Achilles window is a sibling process, not a child of the Claude Code subprocess.

## How the spoken interaction works

The model's reply is divided into three regions. The first region is a short opening sentence that confirms what work is about to start; it is read aloud before any tool calls so the user gets immediate audio feedback that the request landed. The second region is the silent body of the reply, which includes tool calls, code edits, file diffs, intermediate explanations, and tool result summaries; the user reads that in the terminal, they do not hear it. The third region is a closing `<spoken-summary>` block on its own line followed by a closing remark and a closing tag; that block is read aloud once terminal work finishes.
...
When work fails for any reason — a non-zero exit from a tool, a refused permission, an unrecoverable error in the orchestrator — the closing spoken summary begins with the phrase `I ran into a problem` regardless of what the model narrated. The orchestrator inspects the Claude run's exit code and tool result events to determine failure; the model's narration is not authoritative on the failure path. This is informational so the user knows what to expect from the audio; the contract itself lives in the embedded prompt and the orchestrator's failure-detection logic, not in this skill body.

The phrase is fixed text. A pleasant-sounding summary that hides a failed run is the worst possible failure mode for a voice-driven loop because the user has no other channel to learn that the work did not land. When the spoken stream opens with `I ran into a problem`, the user knows to scroll the terminal back to see what went wrong before issuing the next request.

## Privacy

Achilles holds the ElevenLabs API key only in the user's main-process OS keystore. The renderer never sees the key, the npm tarball never contains the key, IPC traffic never carries the key, and local logs never write the key. Outbound network traffic from Achilles goes only to ElevenLabs endpoints for STT and TTS and to the local claude subprocess on the same machine; no audio or transcript content leaves the user's machine except to ElevenLabs. SAFE-01 in REQUIREMENTS.md owns the key surface contract; SAFE-03 owns the broader data-handling contract.

Transcripts are not persisted by default. Phase 14 owns the opt-in `--save-transcripts` flag and the corresponding `achilles transcripts purge` cleanup command; if a transcript history is required for compliance or for the developer's own review, that flag must be set explicitly per the v1.3 hardening release.
